...
public class MovieDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Movie movie = mdao.getMovieByID(movie_id);
            LocationDAO locationDAO = new LocationDAO();
           List<LocationPosition> lpList  = locationDAO.getListLocationPositions(movie_id);
            
            request.setAttribute("movie",movie);
            request.setAttribute("lpList",lpList);
            request.getRequestDispatcher("common/moviedetail.jsp").forward(request, response);
        } catch (SQLException ex) {
            Logger.getLogger(MovieDetailServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
